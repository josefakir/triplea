                            ...
                            <table id="table" class="align-middle mb-0 table table-borderless table-striped table-hover">
                                ...
                                <tbody>
                                    <tbody>
                                        <?php 
                                            $anio = $_GET['anio'];
                                            $luchadores = Capsule::select("select usuarios.id , usuarios.nombre , sum(bookings.precio) as 'conteo' from usuarios left join bookings on usuarios.id = bookings.id_usuario where (fecha >= '$anio-01-01' and fecha <= '$anio-12-31') group by usuarios.id order by conteo DESC;");
